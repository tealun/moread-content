...
- 根据目录名推断教材版本、年级、学期
- 确定输出 JSON 路径
- **读取同系列已有 JSON 样本**：如果目标路径（如 `textbook/fltrp/junior/chen/`）下已存在同版本的其他册数据，必须优先读取该文件作为格式参考
- 向用户确认推断结果，然后开始
...
### Step 6：核对与修正
- **格式对齐检查（关键）**：
  - 读取同系列已有 JSON 样本（如 `grade8_down.json`）
  - 对比字段名、字段类型、字段顺序是否一致
  - 检查 `null` vs `""` vs 缺失字段的差异
  - 检查 `cefr_level` 等可选字段的现有约定（如有样本为 `null` 则保持一致）
  - 检查 `toc` 项的字段集合是否与已有数据一致
  - 检查 `key_vocabulary` 是否包含 `phonetic` 等已有字段
  - 确保 `topic` 的语言（中文/英文）与已有数据一致
- 检查编码问题（UTF-8 乱码）
- 检查 OCR 错误（缺字母、错字母）
- 检查页码一致性
- 检查单词拼写
...
| 格式与已有数据不一致 | `cefr_level` 应为 `null` 而非 `"B1"`；`topic` 语言应与已有样本一致；`phonetic` 字段不可缺失 |
