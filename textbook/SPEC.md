...
> **关键原则：学段是隔离边界。** 以下按学段分组，不按主编合并。初中和高中是完全独立的教材体系。
...
- 必修1 / 必修2 / 必修3
- ⚠️ 选择性必修4~7暂不收录
- 注：虽然主编也是陈琳，但高中和初中是完全不同的教材体系，独立管理
...
> **注意**：陈琳主编的初中版和高中版是**完全不同的教材体系**，此处描述的结构差异是**各自学段内部**的特征，不代表跨学段的关联。

#### 3.5.1 陈琳·初中版：Module → Unit 嵌套

陈琳初中版的顶层是 Module，每个 Module 包含 3 个 Unit。用嵌套结构表示：
...
**说明：** `sub_units` 只在陈琳·初中版出现。陈琳·高中版、孙有中版和人教版的 `units[]` 直接是扁平列表。消费端根据 `meta.editor` + `meta.level` 联合判断是否需要处理 `sub_units`（仅当 `editor === "chen"` 且 `level === "junior"` 时）。
...
| _(无)_ | `sections[]` | **新增** |
| _(无)_ | `meta.editor` + `meta.level` | **新增**（区分陈琳初中/陈琳高中/孙有中等，需联合 level 判断） |
